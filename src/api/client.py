...
class ApiClient:
    ...
    def get_projects(self) -> List[Project]:
        ...
        url = f"{self.base_app_url}/api/projects"
    ...
    def get_project_users(self, project_id: str) -> list:
        ...
        url = f"{self.base_app_url}/api/{project_id}/users"
        response = self.session.get(url)

        if not response.ok:
            raise RuntimeError(
                f"Failed to get users for project {project_id}: {response.status_code} - {response.text}")
    ...
    def get_suites(self, project_id: str) -> list:
        ...
        url = f"{self.base_app_url}/api/{project_id}/suites"
        response = self.session.get(url)

        if not response.ok:
            raise RuntimeError(
                f"Failed to get suites for project {project_id}: {response.status_code} - {response.text}")
    ...
    def delete_project(self, project_id: str) -> None:
        ...
        url = f"{self.base_app_url}/api/projects/{project_id}"
